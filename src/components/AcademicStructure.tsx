  ...
  const normalized = root.departments.map((department) => {
      ...
      branch.sections.forEach((section) => {
        section.students.forEach((student) => {
          ...
          if (!current.some((existing) => existing.studentId === student.id)) {
            current.push(mapStudentRecord(student, department.id, year))
          }
    ...
    const years = [1, 2, 3, 4].map((year) => {
      ...
      const students = existing.length > 0
        ? existing
        : createSeedStudents(department.name, department.id, year)
...
export default function AcademicStructure({
  ...
  const uploadRows = (rows: UploadRow[]) => {
    ...
    pushToast(`Upload complete: ${rows.length} students added`, 'success')
  }
